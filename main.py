...
def station():
    ...
    if request.method=='POST':
        ...
        if dn>0 and dy>=0:
            st="1"
            cursor.execute("SELECT * FROM ev_station where area like %s || city like %s || landmark like %s order by  distance",(prd,prd,prd))
            sdata= cursor.fetchall()
            for d1 in sdata:
                dt=[]
                lat1=float(d1[6])
...
def map2():
    ...
    return render_template('map2.html',msg=msg, data=data)
...
def view_map():
    ...
    return render_template('view_map.html',msg=msg, data=data)
